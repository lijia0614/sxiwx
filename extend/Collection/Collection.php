...
class Collection{
    ...
    public function CollectsList() {
        ...
        $collects = Db("collects")->where(array("id" => $collect_id))->find();
        ...
        $this->success("URL列表已采集完成", "@Collects/CollectsContent", '', 2);
    }
	
	public function CollectsContent(){
    ...
    public function datadown() {
        ...
        $join = array(array(config('database.prefix') . 'collects b', "b.id=a.collects_id", "LEFT"));
        $ary_data = Db("collects_url")->alias('a')->field($fields)->where('`a`.`status`=0')->join($join)->order('`a`.id desc')->find();
        if (count($ary_data) < 1) {
        ...
        //$get_data=QueryList::Query($content_url,$rulesArr,'','UTF-8',$ary_data['collect_content_type'],true)->data;
        ...
        if (isset($get_data[0])) {
			...
			Db('collects_url')->where('id', $ary_data['content_id'])->update(array('status' => 1));
        ...
        $inset_data['create_time'] = date('Y-m-d H:i:s');
        $inset_data['update_time'] = date('Y-m-d H:i:s');
        $inset_data['status'] = 0;
		if(strtolower($table)=='download'){//如果是下载模型则有下载附件的字段
			$inset_data['is_down']=0;
		}
        $table = $ary_data['model'];
        $result = Db($table)->insert($inset_data);
        if ($result) {
            Db('collects_url')->where('id', $ary_data['content_id'])->update(array('status' => 1));
			JsonDie(1, '<li><span class="correct_span">&radic;</span>'.$inset_data['title'] . '<span style="float:right;">采集完成</span></li>');
			//<li><span class="correct_span">&radic;'.$inset_data['title'] . '采集成功</span>
            //$this->success($inset_data['title'] . "采集成功", "@Collects/CollectsContent", '', 1);
        }
    }
    ...
    public function TestCollects() {
        ...
        return view(WEB_PATH . '/Public/Html/collects.tpl');
    }
                ...
                if (!$TestCollects) {
                    $rulesArr[$key][] = function($content) use($collects) {//规则的第四个参数,获取到的内容回调函数
                        ...
                        foreach ($imgs as $img) {
                            ...
                            if ($src) {
                                $img_path_dir = '/Upload/' . $collects['model'] . '/image/' . date('Y') . '/' . date('m') . '/' . date('d') . '/';
    ...
    private function add_collects_url($collects, $urls) {
        if (is_array($urls)) {
            foreach ($urls as $key => $one) {
                $check = Db("collects_url")->where(array('code' => md5($one['url'])))->find();
                if (!$check) {
                    try {
                        $data = array('collects_id' => $collects['id'], 'url' => $one['url'], 'status' => 0, 'create_time' => date('Y-m-d H:i:s'), 'code' => md5($one['url']));
                        if (isset($one['small_img'])) {
                            $data['small_img'] = $one['small_img'];
                        }
                        Db('collects_url')->insert($data);
                    } catch (\Exception $e) {
                        JsonDie(0, $e, 'no', $data = '');
                    }
                }
    ...
    private function downloadImg($imgurl, $collects) {
        ...
        $this->is_resize_fill($localSrc); //判断是否裁剪图片
        $this->is_watermark($localSrc); //判断是否添加水印
        return $localSrc;
    }
