...
class Page {
    ...
    public function __construct($totalRows,$listRows='',$parameter='',$url='') {
        ...
        if(!isset($parm['p'])){
            ...
            $this->nowPage =$parm[$this->varPage];
        }
        if(!empty($listRows)) {
    ...
    public function show() {
        ...
        if($this->url){
			...
			//$url        =   rtrim(urlto('@'.$this->url,'',false),$depr).$depr.'__PAGE__';
        }else{
		    ...
		    $url            =   url(strtolower(MODULE)."/".strtolower(CONTROLLER).'/'.strtolower(ACTION),$parameter);
        }
        //上下翻页字符串
    ...
    public function newshow() {
        ...
        // 分析分页参数
        if($this->url){
            $depr       =   config('app.pathinfo_depr');
            $url        =   rtrim(url('/'.$this->url,'',false),$depr).$depr.'__PAGE__';
        }else{
            if($this->parameter && is_string($this->parameter)) {
                parse_str($this->parameter,$parameter);
            }elseif(empty($this->parameter)){
                unset($_GET[config('app.VAR_URL_PARAMS')]);
                if(empty($_GET)) {
                    $parameter  =   array();
                }else{
                    $parameter  =   $_GET;
            ...
            $parameter[$p]  =   '__PAGE__';
			if(defined("SYS_PATH")){
            	$url =   url(SYS_PATH."/".CONTROLLER."/".ACTION,$parameter);
			}else{
				$url =   url('',$parameter);
			}
        }
        ...
        if($nowCoolPage == 1){
            ...
            $preRow     =   $this->nowPage-$this->rollPage;
            $prePage    =   "<a href='".str_replace('__PAGE__',$preRow,$url)."'  target='_self'>上".$this->rollPage."页</a>";
            $theFirst   =   "<a href='".str_replace('__PAGE__',1,$url)."' target='_self' >".$this->config['first']."</a>";
        }
        if($nowCoolPage == $this->coolPages){
            $nextPage   =   '';
            $theEnd     =   '';
            ...
            $nextRow    =   $this->nowPage+$this->rollPage;
            $theEndRow  =   $this->totalPages;
            $nextPage   =   "<a href='".str_replace('__PAGE__',$nextRow,$url)."' target='_self'>下".$this->rollPage."页</a>";
            $theEnd     =   "<a href='".str_replace('__PAGE__',$theEndRow,$url)."'  target='_self'>".$this->config['last']."</a>";
        }
        // 1 2 3 4 5
        $linkPage = "";
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
                    $linkPage .= "<a href='".str_replace('__PAGE__',$page,$url)."'  target='_self'>".$page."</a>";
                }else{
                    break;
                }
            }else{
