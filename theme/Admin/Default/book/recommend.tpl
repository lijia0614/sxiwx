<div class="nav"><span>编辑力荐管理</span></div>
<div class="explain">
	<div class="name"><span>操作提示</span></div>
	<div class="con">
		<p>推荐管理主要负责推荐管理的书籍(数量8个)</p>
	</div> 
</div>
...
<form method="get"  action="{:url(SYS_PATH.'/'.CONTROLLER.'/index')}" class="ajaxSearchFrom">
	<div class="filter">
		<a href="javascript:void(0);" data-url="{:url(SYS_PATH.'/'.CONTROLLER.'/addRecommend')}" class="add dialog" data-height="500px" data-width="850px" title="添加书籍">添加书籍到小编力荐</a>
	</div>
</form>
	...
	<table width="100%" class="ltable">
        ...
        <volist name="list" id="vo">
					<tr>
						...
						<td>{$vo.author}</td>
						<td><a href="javascript:void(0);" data-url="{:url(SYS_PATH."/".CONTROLLER."/recommend_sort",array("id"=>$vo["id"]))}" class="dialog edit" alt="编辑排序" title="编辑排序" data-height="300px" data-width="550px">{$vo.recommend_sort}</a></td>
						<td>
							 <a href='javascript:void(0);' data-url='{:url(SYS_PATH."/".CONTROLLER."/outRecommend","id=$vo[id]")}' class='remove doDel del' val='{$vo.id}' title='移出此管理'>移出此管理</a>
						</td>
					</tr> 
       </volist>
