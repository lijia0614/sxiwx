...
<body>
  ...
  <div class="pad-10"> 
   ...
   <if condition="isset($id)">
    <form action="{:url(SYS_PATH."/".CONTROLLER."/CategoryEdit",array('id'=>$data.id))}" method="post" class="formvalidate" id="DataFromSubmit" isajax="true">
     ...
     <fieldset> 
      ...
      <table width="100%" class="table_form"> 
       <tbody> 
        ...
        <tr> 
         <td width="12%">分类名称</td> 
         <td><input type="text" id="title"  style="width:300px;" class="input-text topinyin" datatype="*" value="{:isset($data['title'])&&$data['title']?$data['title']:''}" name="title" placeholder="请输入分类名称" /><span id="messageBox" for="name" generated="true" class="Validform_checktip onShow ">分类名称</span> </td>
        </tr>
        ...
        <tr>
         ...
         <td>
           ...
           <select name="status">
            ...
            <option <if condition="$data.status eq 1">selected</if> value="1">正常</option>
            <option <if condition="$data.status eq 0">selected</if> value="0">禁用</option>
           </select>
