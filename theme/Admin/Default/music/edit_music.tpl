...
<div class="fixed-bar">
   ...
   <if condition="isset($id)">
    <form action="{:url(SYS_PATH."/".CONTROLLER."/editMusic",array('id'=>$id))}" method="post" class="formvalidate" id="DataFromSubmit" isajax="true">
    <input type="hidden" name="id" value="{$id}" />
   <else />
   	<form action="{:url(SYS_PATH."/".CONTROLLER."/editMusic")}" method="post" class="formvalidate" id="DataFromSubmit" isajax="true">
   </if>
     <fieldset> 
      <!--<legend>填写信息</legend>-->
           ...
           <td>
               <select id="lang" name="lang" datatype="*" nullmsg="请选择" class="Validform_error">
                   ...
                   <volist name="lang" id="vo">
                       <option style="font-weight:bold;" <if condition="$data['lang'] eq $vo['id']">selected</if> value="{$vo.id}">{$vo.title}</option>
                   </volist>
               </select>
           ...
           <td>
               <select id="type" name="type" datatype="*" nullmsg="请选择" class="Validform_error">
                   ...
                   <volist name="type" id="vo">
                       <option style="font-weight:bold;" <if condition="$data['type'] eq $vo['id']">selected</if>  value="{$vo.id}">{$vo.title}</option>
                   </volist>
               </select>
               <span id="messageBox" for="title" generated="true" class="Validform_checktip onShow Validform_wrong">
           请选择
           ...
           <td>
               <select id="feel" name="feel" datatype="*" nullmsg="请选择" class="Validform_error">
                   ...
                   <volist name="feel" id="vo">
                       <option style="font-weight:bold;" <if condition="$data['feel'] eq $vo['id']">selected</if>  value="{$vo.id}">{$vo.title}</option>
                   </volist>
               </select>
               <span id="messageBox" for="title" generated="true" class="Validform_checktip onShow Validform_wrong">
           请选择
