<div class="nav"><span>黑岩推送书籍列表</span></div>
...
<form action="{:url(SYS_PATH."/".CONTROLLER."/heiYan")}" id="formsubmit" class="formvalidate2">
<div class="main"> 
...
<script language="javascript">
	...
	$('.editLc').click(function () {
		...
		var ids = (id.join(','));
		$.ajax({
			url: "{:url(SYS_PATH."/".CONTROLLER."/heiYan")}",
			data:{"ids":ids},
			type: "POST",
	...
	$('.editStatus').click(function () {
        ...
        $.ajax({
            url: "{:url(SYS_PATH."/".CONTROLLER."/editStatusHy")}",
            data:{"status":status},
            type: "POST",
            success: function (req) {
                console.log(req);
