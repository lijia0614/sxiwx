<div class="nav"><span>落尘推送书籍列表</span></div>
...
<form action="{:url(SYS_PATH."/".CONTROLLER."/luoChen")}" id="formsubmit" class="formvalidate2">
<div class="main"> 
...
<script language="javascript">
	...
	$('.editLc').click(function () {
		...
		var ids = (id.join(','));
		$.ajax({
			url: "{:url(SYS_PATH."/".CONTROLLER."/luoChen")}",
			data:{"ids":ids},
			type: "POST",
	...
	$('.editStatus').click(function () {
        ...
        $.ajax({
            url: "{:url(SYS_PATH."/".CONTROLLER."/editStatus")}",
            data:{"status":status},
            type: "POST",
            success: function (req) {
                console.log(req);
