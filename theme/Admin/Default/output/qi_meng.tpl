<div class="nav"><span>启梦推送书籍列表</span></div>
...
<form action="{:url(SYS_PATH."/".CONTROLLER."/qiMeng")}" id="formsubmit" class="formvalidate2">
<div class="main"> 
...
<script language="javascript">
	...
	$('.editLc').click(function () {
		...
		var ids = (id.join(','));
		$.ajax({
			url: "{:url(SYS_PATH."/".CONTROLLER."/qiMeng")}",
			data:{"ids":ids},
			type: "POST",
	...
	$('.editStatus').click(function () {
        ...
        $.ajax({
            url: "{:url(SYS_PATH."/".CONTROLLER."/editStatusQm")}",
            data:{"status":status},
            type: "POST",
            success: function (req) {
                console.log(req);
