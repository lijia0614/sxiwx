            ...
            <div class="novel-container  yellow">
                ...
                <div class="novel-content">
                    ...
                    <div class="null-content">
                        <div class="null-novel-text" style="font-size: 16px;">
                            {$data['content']}...
                        </div>
                        <if condition="$user['id']">
                            <if condition="$all">
                                <div class="take-chapter ">
                                    <div class="notice">此书为收费书籍，需要订阅后才能阅读哦</div>
                                    <p>本书价格为：{$price} 书币</p>
                                    <p>本次共消费：
                                        <span>
                                    {$price}
                                    ...
                                    <p style="">
                                        账户余额：{$user['money']} 书币
                                        <a target="_blank" href="{:url('recharge/recharge')}">去充值&gt;&gt;</a>
                                    </p>
                                    <input type="hidden" id="z_id" value="{$data['id']}">
                                    <input type="hidden" id="b_id" value="{$data['b_id']}">
                                    <span id="buy_all" class="btn">
                                    订阅本章
                                </span>
                                </div>
                                <else/>
                                <div class="take-chapter ">
                                    <div class="notice">此章为收费章节，需要订阅后才能阅读哦</div>
                                    <p>5 书币/千字（本章字数{$count}字）</p>
                                    <p>本次共消费：
                                        <span>
                                {$price}
                                </span> 书币
                            ...
                            <else/>
                            <div class="login-div">
                                <img src="images/no_login.png" alt="登录">
                                此章节为收费章节，需要
                                <a class="login" target="_blank" href="{:url('login/login',['back'=>$back,'b_id'=>$data['b_id'],'id'=>$data['id']])}">登录</a>
                                并订阅才能阅读
                            </div>
                        </if>
                    </div>
...
<div role="document" class="rc-dialog reward-modal reward-modal-read" style="width: 600px;">
    <div class="rc-dialog-content">
        ...
        <div class="rc-dialog-body">
            ...
            <input id="chapterId" value="{$chapter_id}" type="hidden">
            ...
            layer.confirm('确定购买吗，购买之后不能退款', {
                ...
                $.ajax({
                    ...
                    success:function(req){
                        var c = JSON.parse(req);
                        if (c.status == 0){
                            layer.msg(c.msg, {icon: 2}); return false;
                        }
                        if (c.status == 1){
                            layer.msg(c.msg, {icon: 1});
                            setTimeout(function(){ window.location.href = "{:url('read/read')}"+'&id='+chapterId+'&bookId='+bookId; }, 0);
                        }
                    }
                });
            }, function(){
            ...
            layer.confirm('确定购买吗，购买之后不能退款', {
                ...
                $.ajax({
                    ...
                    success:function(req){
                        ...
                        if (c.status == 1){
                            layer.msg(c.msg, {icon: 1});
                            setTimeout(function(){ window.location.href = "{:url('read/read')}"+'&id='+id+'&b_id='+b_id; }, 0);
                        }
                    }
                });
            }, function(){
